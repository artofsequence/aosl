...
. +name+ is a type representing a valid identifier for an element.
. +name+ must be implemented as an <<aoslspecs_identifier, identifier>>.
. +name+'s value must be interpreted as the identifier of an element, either for reference or definition.
...
. +email+ must be implemented as a <<aoslspecs_string_type,string-type>> with the following additional constraints:
...
. +version+ is a type representing a valid textual identifier (in several contexts).
. +version+ must be implemented as an <<aoslspecs_identifier, identifier>>.
