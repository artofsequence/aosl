...
type::
    Explain what is a type here?

element::
    Explain what is an element here?
...
. unit_space must have at least the same behaviour than the double type.
. Spatial units must be implemented relative to the definition of the *canvas*.
...
. unit_time must have at least the same behaviour than the double type.
. Time units must be implemented as seconds.
